...
n=$#
if [[ $n -ne 7 ]]; then
    echo usage : PlotZllSequence.bash [ERA] [CHANNEL] [GENERATOR] [IP_CUT1] [IP_CUT2] [IP_SF] [CALIBR_DY]
    echo ERA = [Run3, Run3_2022, Run3_2023]
    echo CHANNEL = [mm,ee]
    echo GENERATOR = [amcatnlo,MG,powheg]
    echo IP_CUT1 = [0,1] 
    echo IP_CUT2 = [0,1] 
    echo IP_SF = [0,1]
    echo CALIBR_DY = DY normalization scale factor, default = 1.0
    exit
fi
...
./scripts/PlotZll.py --era ${era} --channel ${chan} --variable pt_2 --nbins 30 --xmin 0 --xmax 150 --applyIP1 ${applyIP1} --applyIP2 ${applyIP2} --applySF ${applySF} --generator ${generator} --calibrDY ${calibrDY}
./scripts/PlotZll.py --era ${era} --channel ${chan} --variable eta_1 --nbins 24 --xmin -2.4 --xmax 2.4  --applyIP1 ${applyIP1} --applyIP2 ${applyIP2} --applySF ${applySF} --generator ${generator} --calibrDY ${calibrDY}
./scripts/PlotZll.py --era ${era} --channel ${chan} --variable eta_2 --nbins 24 --xmin -2.4 --xmax 2.4 --applyIP1 ${applyIP1} --applyIP2 ${applyIP2} --applySF ${applySF} --generator ${generator} --calibrDY ${calibrDY}
