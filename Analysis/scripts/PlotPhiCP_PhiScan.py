...
def Plot(hists,**kwargs):
    ...
    leg.SetHeader('#phi scan A=%5.3f'%(asym))
    ...
    leg1.SetHeader('no #phi scan A=%5.3f'%(asym2))
    ...
    outputGraphics = graphicFolder+'/'+var+'_'+chan+'_'+era+'_phi.png'    
...
if __name__ == "__main__":
    ...
    filename = '%s/%s_%s_%s_cpp_noPhiScan.root'%(basefolder,sample,chan,era)
    filename2 = '%s/%s_%s_%s_cpp_phiScan.root'%(basefolder,sample,chan,era)
    ...
    inputfile2 = ROOT.TFile(filename2,'read')

    bins = utils.createBins(nbins,xmin,xmax)
    
    name_sm = '%s_%s_KinFit_sm'%(sample,var)
    name_ps = '%s_%s_KinFit_ps'%(sample,var)
    name_2_sm = '%s_%s_KinFit_sm'%(sample,var)
    name_2_ps = '%s_%s_KinFit_ps'%(sample,var)
    hist_ps = utils.rebinHisto(inputfile.Get(name_sm),bins,'_rebinned_ps')
    hist_sm  = utils.rebinHisto(inputfile.Get(name_ps),bins,'_rebinned_sm')
    hist_2_ps = utils.rebinHisto(inputfile2.Get(name_2_sm),bins,'_rebinned_ps_2')
    hist_2_sm  = utils.rebinHisto(inputfile2.Get(name_2_ps),bins,'_rebinned_sm_2')
    ...
    Plot(hists,era=era,var=var,channel=chan,asym=asym2,asym2=asym,ymin=ymin,ymax=ymax)
