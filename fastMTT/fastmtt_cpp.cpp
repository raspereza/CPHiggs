...
map<string, py::array_t<double>> fastmtt_cpp(unsigned int N,
  ...
  double mX_min = mX - widthX;
  double mX_max = mX + widthX;
  ...
  auto x_1_cons_vec = py::array_t<double>(buffer.size);
  auto x_2_cons_vec = py::array_t<double>(buffer.size);
  ...
  py::buffer_info x_1_cons_buffer = x_1_cons_vec.request();
  py::buffer_info x_2_cons_buffer = x_2_cons_vec.request();
  ...
  double * x_1_cons = static_cast<double *>(x_1_cons_buffer.ptr);
  ...
  for (unsigned int i=0; i<N; ++i) {  
    ...
    double min_likelihood_cons = 0.;
    double x1_opt_cons = 0.02;
    double x2_opt_cons = 0.02;
    
    // scan over weights for each ditau four-vector

    for (unsigned int i1 = 1; i1 < 50; ++i1) {
      double x1 = 0.02*double(i1);
	...
	x2_opt_cons = x2;
      ...
      for (unsigned int i2 = 1; i2 < 50; ++i2) {
	double x2 = 0.02*double(i2);
	double test_mass = m_vis/sqrt(x1*x2);
	double x1_min = min(double(1.0), (m_vis_1/m_tau)*(m_vis_1/m_tau));
	double x2_min = min(double(1.0), (m_vis_2/m_tau)*(m_vis_2/m_tau));
	...
	if (test_mass>mX_min && test_mass<mX_max) {
    ...
    bool boundary_BW = ((x1_opt_BW<0.021)||(x1_opt_BW>0.979))&&((x2_opt_BW<0.021)||(x2_opt_BW>0.979));  
    bool boundary_cons = ((x1_opt_cons<0.021)||(x1_opt_cons>0.979))&&((x2_opt_cons<0.021)||(x2_opt_cons>0.979));
    ...
    if (boundary_cons) {
      x1_opt_cons = x1_opt_BW;
      x2_opt_cons = x2_opt_BW;
    }
    ...
    x_1_BW[i] = x1_opt_BW;
    x_2_BW[i] = x2_opt_BW;
    x_1_cons[i] = x1_opt_cons;
    x_2_cons[i] = x2_opt_cons;
  ...
  map<string, py::array_t<double> > results = {
    ...
    {"x1_BW",x_1_BW_vec},
    {"x2_BW",x_2_BW_vec},
    {"x1_cons",x_1_cons_vec},
    {"x2_cons",x_2_cons_vec},
    {"mass",mass_vec}
  };
  return results;
...
PYBIND11_MODULE(fastmtt_cpp, m) {
  m.def("fastmtt_cpp", fastmtt_cpp, "FastMTT");
}
